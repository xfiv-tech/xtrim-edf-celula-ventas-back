...
async def RegistrarIsla(data:Isla):
    try:
        print("RegistrarIsla")
        db.execute(isla.insert().values(
            id_ciudad = data.id_ciudad,
            isla = data.isla
        ))
        return {
            "status": 200,
            "message": "Isla registrada correctamente"
        }
    except Exception as e:
        print(e)
        return {
            "status":400,
            "message": "Error al registrar la isla"
        }
...
async def ListarIslaAll():
    try:
        ...
        return {
            "status":400,
            "message": "Error al listar las islas"
        }
...
async def UpdateIsla(data:Isla):
    try:
        print("UpdateIsla")
        db.execute(isla.update().where(isla.c.id == data.id).values(
            id_ciudad = data.id_ciudad,
            isla = data.isla
        ))
        return {
            "status": 200,
            "message": "Isla actualizada correctamente"
        }
    except Exception as e:
        print(e)
        return {
            "status":400,
            "message": "Error al actualizar la isla"
        }
...
async def DeleteIsla(id:int):
    try:
        ...
        return {
            "status":400,
            "message": "Error al eliminar la isla"
        }