...
class SQLModelSelector(Generic[SchemaModelT]):
    ...
    def __init__(
        self, model: Type[SchemaModelT] = None, fields: List[SQLModelListField] = None
    ) -> None:
...
class SQLModelCrud(BaseCrud, SQLModelSelector):
    ...
    def _create_schema_list(self) -> Type[SchemaListT]:
        ...
        modelfields = list(
            filter(
                None,
                [
                    self.parser.get_modelfield(sqlfield, clone=True)
                    for sqlfield in self._select_entities.values()
                ],
            )
        )
    ...
    def _create_schema_filter(self) -> Type[SchemaFilterT]:
        ...
        list_filter = (
            self.parser.filter_insfield(self.list_filter, save_class=(Label,))
            or self._select_entities.values()
        )
        modelfields = list(
            filter(
                None,
    ...
    def _create_schema_read(self) -> Optional[Type[SchemaReadT]]:
        ...
        self.read_fields = self.parser.filter_insfield(
            self.read_fields, save_class=(ModelField,)
        )
        modelfields = [
            self.parser.get_modelfield(ins, clone=True) for ins in self.read_fields
        ]
        return schema_create_by_modelfield(
            f"{self.schema_name_prefix}Read", modelfields, orm_mode=True
        )
    ...
    def _create_schema_update(self) -> Type[SchemaUpdateT]:
        ...
        modelfields = [
            self.parser.get_modelfield(ins, clone=True) for ins in self.update_fields
        ]
        if self.update_exclude is None:  # deprecated in version 0.4.0
            exclude = {self.pk_name} | {
                self.parser.get_modelfield(ins).name
                for ins in self.parser.filter_insfield(
                    self.readonly_fields
                )  # readonly fields, deprecated
            }
        else:
            exclude = {
                k
    ...
    def _create_schema_create(self) -> Type[SchemaCreateT]:
        ...
        modelfields = list(
            filter(
                None,
                [
                    self.parser.get_modelfield(field, clone=True)
    ...
    def _fetch_item_scalars(
        self, session: Session, item_id: List[str]
    ) -> List[SchemaModelT]:
    ...
    async def on_filter_pre(
        self, request: Request, obj: SchemaFilterT, **kwargs
    ) -> Dict[str, Any]:
        return obj and {
            k: v
    ...
    def filtered_item_id(self) -> Callable:
        ...
        async def depend(
            ...
            filtered_id = await self.db.async_scalars(
                stmt.where(self.pk.in_(item_id)).with_only_columns(self.pk)
            )
