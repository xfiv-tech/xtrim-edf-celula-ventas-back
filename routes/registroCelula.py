...
from redisConexion.RedisQuerys import (
    AddRedis,
    DeleteRedis,
    GetterRedis,
    SetterRedis,
    ...
    try:
        ...
        for i in ciudad:
            query = (
                ...
                .join(
                    Operador, RegistrarVendedor.c.id_operador == Operador.c.id_operador
                )
                .join(
                    SistemaOperativo,
                ...
                .join(
                    Modalidad,
                    RegistrarVendedor.c.id_modalidad == Modalidad.c.id_modalidad,
                )
                .select()
                .with_only_columns(
                    Channel.c.channel,
        ...
        for i in data:
            if (
                ...
                query_jefe_venta = (
                    RegistroJefeVentas.select()
                    .where(RegistroJefeVentas.c.id_jefe_venta == i.id_jefe_venta)
                    .with_only_columns(
...
async def get_registroID(id_registrar_vendedor: int, request: Request):
    try:
        if id_registrar_vendedor == 0 or id_registrar_vendedor == None:
            return {
                "code": "-1",
                "error": "id_registrar_vendedor no puede ser 0 o nulo",
            }
        header = request.headers
        ...
        for i in data:
            ...
            if dataInfo:
                ...
                return {"code": "1", "data": "No se encontraron datos"}
    except Exception as e:
        return {"error": str(e)}
...
async def post_registro(request: RegistrarVendedorModel):
    try:
        ...
        query = RegistrarVendedor.insert().values(
            ...
            id_lider_peloton=(
                0 if request.id_lider_peloton == None else request.id_lider_peloton
            ),
    ...
    try:
        ...
        query = (
            ...
            .values(
                ...
                id_gerente_zonal=request.id_gerente_zonal,
                dias_inactivo=(
                    0 if request.dias_inactivo == None else request.dias_inactivo
                ),
            )
            .where(
                RegistrarVendedor.c.id_registrar_vendedor
...
async def get_distribuidor():
    try:
        query = (
            RegistrarDistribuidor.join(
                Estados, Estados.c.id_estado == RegistrarDistribuidor.c.id_estado
            )
            .select()
            .with_only_columns(
            ...
            infoData.append(
                {
                    ...
                    "fecha_ingreso": i.fecha_ingreso,
                    "fecha_salida": i.fecha_salida,
                    "ciudades_asignadas": await ListarCiudadesDistribuidor(
                        i.id_registrar_distribuidor
                    ),
                    "canales_asignados": await ListarCanalesDistribuidor(
                        i.id_registrar_distribuidor
                    ),
                }
            )
...
async def put_distribuidor(request: RegistrarDistribuidorModel):
    try:
        query = (
            ...
            .where(
                RegistrarDistribuidor.c.id_registrar_distribuidor
                == request.id_registrar_distribuidor
            )
        )
        data = db.execute(query)
        return {"code": "0", "data": data}
...
@registro.delete(
    "/eliminar_distribuidor/{id_registrar_distribuidor}", tags=["Distribuidor"]
)
                ...
                infoData.append(
                    {
                        ...
                        "ciudades_asignadas": await ListarCiudadesJVCiudad(
                            i.id_jefe_venta
                        ),
                        "canales_asignados": await ListarCanalesJVCiudad(
                            i.id_jefe_venta
                        ),
                    }
                )
...
async def get_administrador():
    try:
        query = (
            RegistroAdministrador.join(
                Estados, Estados.c.id_estado == RegistroAdministrador.c.id_estado
            )
            .select()
            .with_only_columns(
            ...
            infoData.append(
                {
                    ...
                    "ciudades_asignadas": await ListarCiudadesAdminCiudad(
                        i.id_administrador
                    ),
                    "canales_asignados": await ListarCanalesAdminCiudad(
                        i.id_administrador
                    ),
                }
            )
            ...
            infoData.append(
                {
                    ...
                    "telefono": i.telefono,
                    "cedula": i.cedula,
                    "ciudades_asignadas": await ListarCiudadesGRegional(
                        i.id_gerente_regional
                    ),
                    "canales_asignados": await ListarCanalesGRciudad(
                        i.id_gerente_regional
                    ),
                }
            )
...
async def put_gerente_regional(request: RegistrarGerenteRegionalModel):
    try:
        db.execute(
            RegistrarGerenteRegional.update()
            .values(
                id_channel=request.id_channel,
                id_ciudad=request.id_ciudad,
                id_estado=request.id_estado,
...
@registro.delete(
    "/eliminar_gerente_regional/{id_gerente_regional}", tags=["Gerente Regional"]
)
            ...
            infodata.append(
                {
                    ...
                    "telefono": i.telefono,
                    "cedula": i.cedula,
                    "ciudades_asignadas": await ListarCiudadesGCiudad(
                        i.id_gerente_ciudad
                    ),
                    "canales_asignados": await ListarCanalesGCiudad(
                        i.id_gerente_ciudad
                    ),
                }
            )
...
@registro.delete(
    "/eliminar_gerente_ciudad/{id_gerente_ciudad}", tags=["Gerente de Ciudad"]
)
...
async def get_administrador_proyectos():
    try:
        query = (
            RegistrarAdminProyectos.join(
                Estados, Estados.c.id_estado == RegistrarAdminProyectos.c.id_estado
            )
            .select()
            .with_only_columns(
            ...
            infoData.append(
                {
                    ...
                    "ciudades_asignadas": await ListarCiudadesAPCiudad(
                        i.id_admin_proyectos
                    ),
                    "canales_asignados": await ListarCanalesAPCiudad(
                        i.id_admin_proyectos
                    ),
                }
            )
...
@registro.put(
    "/actualizar_administrador_proyectos", tags=["Administrador de Proyectos"]
)
async def put_administrador_proyectos(request: RegistrarAdminProyectosModel):
    try:
        data = db.execute(
            RegistrarAdminProyectos.update()
            .values(
                id_channel=request.id_channel,
                id_ciudad=request.id_ciudad,
                id_estado=request.id_estado,
...
@registro.delete(
    "/eliminar_administrador_proyectos/{id_admin_proyectos}",
    tags=["Administrador de Proyectos"],
)
...
async def cargar_excel_vendedores(request: Request):
    try:
        ...
        if extension == "xlsx":
            ...
            return {"code": "1", "message": "El archivo debe ser de tipo excel"}
        nueva_lista = []
        for i in df:
            ...
            i["id_gerente_regional"] = (
                None
                if i["id_gerente_regional"] == "SIN REGISTRO"
                else int(i["id_gerente_regional"])
            )
            i["usuario_equifax"] = (
                None if i["usuario_equifax"] == "SIN USUARIO" else i["usuario_equifax"]
            )
            i["fecha_ingreso"] = (
                None if i["fecha_ingreso"] == "SIN REGISTRO" else i["fecha_ingreso"]
            )
            i["id_lider_peloton"] = (
                0 if i["id_lider_peloton"] == "SIN REGISTRO" else i["id_lider_peloton"]
            )
            i["id_jefe_venta"] = (
                None if i["id_jefe_venta"] == "SIN REGISTRO" else i["id_jefe_venta"]
            )
            i["fecha_salida"] = None if i["fecha_salida"] == "SIN REGISTRO" else None
            i["sector_residencia"] = (
                "SIN REGISTRO"
                if i["sector_residencia"] == "SIN REGISTRO"
                else str(i["sector_residencia"])
            )
...
async def descargar_excel_prosupuesto():
    try:
        ...
        return FileResponse(
            "static/ejemplo_prosupuesto.xlsx",
            media_type="application/octet-stream",
            filename="ejemplo_prosupuesto.xlsx",
        )
    except Exception as e:
        return {"error": str(e)}
